# COPA Airlines Use Case — Session History

Complete chronological history of the prompts and file actions that produced the COPA Airlines Passenger Services AI Agent demo apps in `C:\Work\VsCode\Agentic_AI\Copa_Airlines_Use_Case\`.

---

## Sessions Involved

The COPA work was split across **two sessions** launched from different working directories.
...
- **COPA work date range:** 2026-05-21 -> 2026-05-21
...
- **13:07:18** *(sess A)*  `Bash`  →  `Copa_Airlines_Use_Case/"`
- **13:08:52** *(sess A)*  `Write`  →  `Copa_Airlines_Use_Case/copa-poc.md`
...
- **13:11:12** *(sess A)*  `Read`  →  `Copa_Airlines_Use_Case/copa-poc.md`
- **13:11:16** *(sess A)*  `Read`  →  `Copa_Airlines_Use_Case/copa-poc.md`
- **13:12:19** *(sess A)*  `Write`  →  `Copa_Airlines_Use_Case/database.sql`
- **13:12:47** *(sess A)*  `Write`  →  `Copa_Airlines_Use_Case/reset_data.sql`
- **13:13:23** *(sess A)*  `Write`  →  `Copa_Airlines_Use_Case/swagger.json`
- **13:15:48** *(sess A)*  `Write`  →  `Copa_Airlines_Use_Case/copa-rest-api.flogo`
- **13:16:54** *(sess A)*  `Write`  →  `Copa_Airlines_Use_Case/copa-mcp-server.flogo`
- **13:17:54** *(sess A)*  `Write`  →  `Copa_Airlines_Use_Case/copa-agent.flogo`
- **13:18:07** *(sess A)*  `Bash`  →  `Copa_Airlines_Use_Case/" && python -c "import json; json.load(open('copa-rest-api.flogo')); print('copa-rest-api.flogo: VALID')" && python -c "import json; json.load(open(`
- **13:31:47** *(sess A)*  `Bash`  →  `Copa_Airlines_Use_Case/"`
#### Prompt — 13:37:59 UTC   *(session B)*

> Create a flogo rest service app -  copa-rest-api-demo from this api spec - C:\Work\VsCode\Agentic_AI\Copa_Airlines_Use_Case\swagger.json ... use dummy values in return/reply activities wherever required

- **13:38:06** *(sess B)*  `Read`  →  `Copa_Airlines_Use_Case/swagger.json`
- **13:38:18** *(sess B)*  `Bash`  →  `Copa_Airlines_Use_Case/"`
- **13:38:29** *(sess B)*  `Read`  →  `Copa_Airlines_Use_Case/copa-rest-api.flogo`
- **13:38:43** *(sess B)*  `Bash`  →  `Copa_Airlines_Use_Case/copa-rest-api.flogo', 'utf8'));

// Update name
app.name = 'copa-rest-api-d`
#### Prompt — 13:45:51 UTC   *(session B)*

> I do not see schemas created ...nor schemas configured in REST Trigger request/reply settings ... Return activity flow output message field is also not mapped .... pls check for all the missing configurations .... this app is not at all usable .....look at how rest trigger, return activity is configured C:\Work\VsCode\Agentic_AI\Hospital_AI-Agent_Use_Case\endevour-api.flogo

- **13:49:44** *(sess B)*  `Read`  →  `Copa_Airlines_Use_Case/copa-rest-api-demo.flogo`
- **13:49:52** *(sess B)*  `Grep`  →  `Copa_Airlines_Use_Case/copa-rest-api-demo.flogo`
...
- **13:55:03** *(sess B)*  `Read`  →  `Copa_Airlines_Use_Case/copa-rest-api-demo.flogo`
- **13:55:09** *(sess B)*  `Edit`  →  `Copa_Airlines_Use_Case/copa-rest-api-demo.flogo`
- **13:55:14** *(sess B)*  `Edit`  →  `Copa_Airlines_Use_Case/copa-rest-api-demo.flogo`
- **13:55:19** *(sess B)*  `Edit`  →  `Copa_Airlines_Use_Case/copa-rest-api-demo.flogo`
...
- **14:59:25** *(sess B)*  `Write`  →  `Copa_Airlines_Use_Case/copa-rest-service-demo2.flogo`
- **14:59:37** *(sess B)*  `Bash`  →  `Copa_Airlines_Use_Case/copa-rest-service-demo2.flogo', 'utf8')); console.log('Valid JSON'); consol`

---

## Files Produced

From this session, the following files were created in `Copa_Airlines_Use_Case/`:

| File | Purpose |
|---|---|
| `copa-poc.md` | Demo blueprint - architecture, MCP tools, REST API specs, scenario walkthrough |
| `copa-rest-api.flogo` | Mock REST backend (flight status, bookings, rebooking) - port 3000 |
| `copa-rest-api-demo.flogo` | Earlier version of the REST backend |
| `copa-rest-service-demo2.flogo` | Second iteration of the REST backend |
| `copa-mcp-server.flogo` | MCP Server exposing 3 AI-callable tools - port 9091 |
| `copa-agent.flogo` | AI Agent with OpenAI GPT-4o + WebSocket chat - port 8082 |
